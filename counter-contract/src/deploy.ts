/**
 * Standalone deploy script for the Anonymous Stalking Report Registry contract.
 *
 * Usage:
 *   npm run build && npm run deploy
 *
 * Prompts for a hex seed (or generates a new one), builds a wallet,
 * waits for sync + funds + dust, deploys the registry contract,
...
async function main() {
  ...
  console.log('\n╔══════════════════════════════════════════════════════════╗');
  console.log('║   Anonymous Stalking Report Registry - Deploy Script    ║');
  ...
  // --- Deploy ---
  const contract = await withStatus('Deploying stalking report registry contract (this may take a few minutes)', async () => {
